...
check("골든 데모 불변: 급여계좌 변경 + 예외 체크 → 여전히 PASS(F04는 이번에 고치지 않음)",
      r_salary_exc["decision"] == "PASS")

r_term = client.post("/api/evaluate", json={"action_type": "PRODUCT_TERMINATION"}).get_json()
check("골든 데모 불변: 상품 해지(기관 미지정) → 여전히 HOLD/KB_SAVINGS_LOAN_HOLD(F02는 이번에 고치지 않음)",
      r_term["decision"] == "HOLD" and r_term["matched_rules"] == ["KB_SAVINGS_LOAN_HOLD"])
...
check("F16: 하나은행 이력 조건은 가입 전 확정 사실 — 지금 해지해도 PASS(false HOLD 제거)",
      r_hana["decision"] == "PASS" and "가입 시점에 이미 확정" in r_hana["reason"])
...
check("F16: 다른 규칙(KB_SAVINGS_LOAN_HOLD)에는 영향 없음 — 여전히 HOLD",
      r_term["decision"] == "HOLD")
